...
<!-- Komentarų skiltis -->
<div class='container w-75 mt-4'>
    <p class='h3' id='comments_number_text'><?= $comment_count; ?> comments</p>
    <div id='all_comments'>
...
<script>
    ...
    function CreateReply(comment_id,name,text,date,email,load){
        ...
        if(load == 0){
            $.ajax({
                method: "POST",
                url: "<?php echo site_url('/home/save_reply'); ?>",
                data: {
                    c_id : comment_id,
                    name : name,
                    email : email,
    ...
    function CreateComment(name,email,text,date,load){
        ...
        if(load==0){
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo site_url('/home/save_comment'); ?>",
                data: {
                    name : name,
                    email : email,
                    text : text,
    ...
    <?php foreach($comments as $comment) : ?>
        CreateComment('<?= $comment['name']; ?>','<?= $comment['email']; ?>','<?= str_replace(array("\r\n", "\n", "\r"), ' ', htmlspecialchars(htmlentities(addslashes($comment['comment'])))); ?>','<?= $comment['date']; ?>','<?= $comment['id']; ?>');
    <?php endforeach; ?>
    <?php foreach($replies as $reply) : ?>
        CreateReply('<?= $reply['comment_id']; ?>','<?= $reply['name']; ?>','<?= str_replace(array("\r\n", "\n", "\r"), ' ', htmlspecialchars(htmlentities(addslashes($reply['text'])))); ?>','<?= $reply['date']; ?>','<?= $reply['email']; ?>','1');
    <?php endforeach; ?>
</script>